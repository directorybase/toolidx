...
date: 2026-06-05
note: v3 was Gemini's audit-only close-out of v2. v4 is the content revision incorporating Codex's independent second review of v2.
---

# toolidx.dev — Agent-Readiness Plan (isitagentready.com) — v4

**Branch:** seo-maintenance-v5
**Worker entry:** `src/index.ts` (Hono + Chanfana)
**Constraint:** Additive only — **do not change existing `/v1/*` API logic**. New routes, new middleware that only *adds* headers, new static files, and edits to the `robots.txt`/`llms.txt` string content. The one deliberate exception (see §0a) is a narrow *exclusion* guard added to the existing `last_updated` JSON middleware so it does not pollute new `/.well-known/` JSON bodies — this does not alter behavior for any existing route.
...
**Chosen fix (additive, honors the constraint):** add a single early-return exclusion to the *top* of the existing middleware:

```ts
app.use("*", async (c, next) => {
  await next();
  const path = new URL(c.req.url).pathname;
  if (path.startsWith("/.well-known/")) return; // new: keep spec JSON bodies clean
...
This changes behavior for **no existing route** (all `/.well-known/*` JSON routes are new), so it does not violate "don't change `/v1/*` API logic." If the operator prefers zero edits to that middleware, the alternative is to give each new well-known JSON route a body the validator tolerates — but no standard JSON media type evades the substring check, so the guard is the clean path. **Flagged for operator sign-off.**
...
| 0a | `last_updated` middleware `/.well-known/` skip-guard | edit existing middleware (exclusion only) | enables §4/§6 | high |
...
3. **Tests:** run `tests/` for `/v1/*` regression. Add route tests per new well-known path + a `last_updated`-pollution test (assert excluded for `/.well-known/*`, still present for `/v1/*`) + a middleware test that the `Link` re-wrap doesn't throw on a constructed `Response`.
